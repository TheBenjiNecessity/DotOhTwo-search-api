...
public class SearchService {
    ...
    public List<User> searchUsers(String text, int limit) {
        return scan(USERS_KEY, text, limit, (name, id) -> new User(id, name));
    }

    public List<Reviewable> searchReviewables(String text, int limit) {
        return scan(REVIEWABLES_KEY, text, limit, (title, id) -> new Reviewable(id, title));
    }

    private <T> List<T> scan(String key, String text, int limit, BiFunction<String, String, T> mapper) {
